...
const ProductInfo: FC<ProductInfoProps> = ({
  ...
  const tabs = [
    ...
    { key: 'ingredients', label: 'Состав', content: ingredients },
    {
      key: 'brand',
      label: 'О бренде',
      content: brand && <BrandInfo brand={brand} />,
    },
    {
      key: 'reviews',
      label: `Отзывы ${reviews?.length || 0}`,
    ...
    <div className="flex flex-col mt-28 mx-72">
      <div className="flex gap-x-8">
        {tabs.map((tab) => (
          ...
          >
            <span
              className={`border-transparent pb-2 hover:text-black ${selectedTab === tab.key ? 'border-b-[2px] border-b-black text-black font-bold' : ' text-grey'}`}
            >
              {tab.label}
            </span>
          </div>
      ...
      <div className="mt-10" style={{ overflow: 'hidden' }}>
        <div
          ...
          style={{
            ...
            transform: `translateX(-${tabs.findIndex((tab) => tab.key === selectedTab) * (100 / tabs.length)}%)`,
...
const Reviews: FC<ReviewProps> = ({ reviews }) => {
  const [showMoreCount, setShowMoreCount] = useState(3) // Initially show 3 reviews

  const handleShowMore = () => {
    setShowMoreCount(reviews.length) // Show all reviews on click
  }

  const handleShowLess = () => {
    setShowMoreCount(Math.min(showMoreCount, 3)) // Ensure max of 3 reviews
  }

  const displayedReviews = reviews.slice(0, showMoreCount)
  ...
  return (
    <div className="flex flex-col gap-y-10">
      ...
      <div className="flex items-center">
        {reviews.length > showMoreCount ? ( // Show "Show More" button if more reviews exist
          <button className="text-left primary-hover" onClick={handleShowMore}>
            Показать больше
          </button>
        ) : showMoreCount > 3 ? ( // Show "Show Less" button only if more than 3 are shown
          <button className="text-left primary-hover" onClick={handleShowLess}>
            Показать меньше
          </button>
        ) : null // Don't render button if initial count or less is shown
        }
        {reviews.length > showMoreCount && (
          <BotIcon
            className={`transition-transform duration-300 ml-2 ${showMoreCount > 3 ? 'rotate-180' : 'rotate-0'}`}
            color="var(--black)"
            strokeWidth={2}
            size={18}
          />
