    ...
    <div className={`flex ${className}`}>
      <div className="flex flex-col items-center mr-[53px] h-[110%]">
        <button onClick={handlePrev} disabled={currentSlide === 0}>
          <PrevArrowIcon
            color={prevButtonDisabled ? 'var(--grey)' : 'var(--black)'}
        ...
        <button
          onClick={handleNext}
          disabled={currentSlide + 4 >= images.length}
        >
          <ArrowIcon
            color={nextButtonDisabled ? 'var(--grey)' : 'var(--black)'}
            className="rotate-90 mt-4"
          />
        </button>
      </div>
      <div className="relative">
        ...
        <div className="flex absolute top-0 left-0 font-sans font-meduim leading-5">
          {isNew && <p className="bg-neon_green py-0.5 px-2">NEW</p>}
          {discount && discount > 0 && (
            <p className="bg-orange text-white py-0.5 px-2">{discount}%</p>
          )}
          {isHit && <p className="bg-gold text-black py-0.5 px-2">HIT</p>}
        </div>
      </div>
    </div>
  )
