        ...
        <>
            <div className="flex justify-between items-end">
                <div className={`flex cursor-pointer items-center`} onClick={toggleDropdown}>
                    ...
                    <h3 className={`${className ?? "text-22px font-semi-bold"}`}>{name}</h3>
                    {count > 0 && 
                        <p className='font-semibold bg-primary text-white rounded-full px-2 ml-2'>{count}</p>
                    }
                    {iconPosition === 'after' && (
                        <BotIcon 
                ...
                {button && <button onClick={unCheck} className='text-16px text-grey hover:text-black duration-300'>сбросить</button>}
            </div>
