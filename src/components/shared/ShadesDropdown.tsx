...
const ShadesDropdown: FC<ShadesDropdownProps> = ({ shades, className }) => {
  ...
  const [selectedShade, setSelectedShade] = useState<ShadeOption | null>(
    shadesAsObjects[0],
  )
  ...
  return (
    <div className="w-80 relative flex flex-col items-center">
      <div
        className="w-full mt-7 flex items-center justify-between cursor-pointer border-b-[1px]"
        onClick={toggleDropdown}
      >
        <div className="flex items-center">
          <div
            className={`rounded-full w-4 h-4 mr-2`}
            style={{ backgroundColor: selectedShade?.data }}
          ></div>
      ...
      <div
        className={`absolute w-[110%] top-[50px] bg-white shadow-xl transition-all duration-500 ease-linear overflow-hidden ${isOpen ? 'max-h-96' : 'max-h-0'}`}
      >
        {shadesAsObjects.map((shade, index) => (
          <div
            key={index}
            className={`flex items-center cursor-pointer px-4 ${index === shadesAsObjects.length - 1 ? 'pb-4' : ''} ${index === 0 ? 'pt-4' : ''}`}
            onClick={() => {
              handleSelect(shade)
            }}
          >
            <div
              className={`rounded-full w-4 h-4 mr-2 `}
              style={{ backgroundColor: shade.data }}
            ></div>
            <p
              className={`text-[14px] primary-hover ${selectedShade?.value === shade.value ? 'text-primary' : ''}`}
            >
              {shade.label}
            </p>
          </div>
