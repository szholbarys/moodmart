...
const Button: React.FC<ButtonProps> = ({
  ...
  return (
    <button
      onClick={onClick}
      className={`${combinedClasses} ${className} duration-300 ease-in-out flex items-center text-16px ${type === 'transparent' ? 'group hover:text-primary hover:transition duration-200 ease-in-out' : ''}`}
      type="button"
    >
      {children}
      {noArrow && (
        <ArrowIcon
          color={iconColor}
          className={`${type === 'transparent' ? 'group-hover:fill-primary ml-2 duration-200 ease-in-out' : 'ml-2'}`}
        />
      )}
    </button>
  )
