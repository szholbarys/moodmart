...
const CheckBoxTree: React.FC<CheckBoxTreeProps> = ({ className = '', setUncheck, setMainCategoryCount }) => {
        ...
        setCheckedIds(currentIds => {
            ...
            if (category && hasSubcategories && category.subcategories) {
                ...
                if (currentIndex === -1) {
                    ...
                    newChecked = newChecked.filter(cid => !category.subcategories!.some(sub => sub.id === cid));
                }
            } else if (category && !hasSubcategories) {
                // A subcategory is toggled, check if we should toggle the parent
                const parentCategory = categories.find(cat => cat.subcategories?.some(sub => sub.id === id));
                if (parentCategory && parentCategory.subcategories) {
                    const areAllSubsChecked = parentCategory.subcategories.every(sub => newChecked.includes(sub.id));
                    if (areAllSubsChecked) {
                        // All subcategories are checked: check the parent
                        newChecked.push(parentCategory.id);
                    } else if (parentCategory.subcategories.some(sub => !newChecked.includes(sub.id))) {
    ...
    const calculateCheckedMainCategories = () => {
        ...
        categories.forEach(category => {
            ...
            if (checkedIds.includes(category.id)) {
                ...
                const isAnySubChecked = category.subcategories.some(sub => checkedIds.includes(sub.id));
                if (isAnySubChecked) {
                    count += 1;
                }
            }
            ...
            {category.subcategories ? (
              <div className="gap-2 mb-1">
                <input
                  ...
                  onChange={() => handleCheckboxChange(category.id, !!category.subcategories)}
                  className={`${partialIds.includes(category.id) ? 'partial' : ''} float-left mt-[5.5px] mr-2`}
                />
                <DropdownFilter iconPosition='after' className='font-medium text-18px' initOpen={true} name={category.name}>
