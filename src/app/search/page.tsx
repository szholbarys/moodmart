...
const SearchResults: React.FC = () => {
  ...
  return (
    <div
      className={`mt-40 mx-20 ${filteredProducts.length === 0 ? 'h-screen' : 'h-full'}`}
    >
      ...
      {filteredProducts.length === 0 ? (
        ...
        <Fragment>
          <Suspense
            fallback={
              <div className="animate-pulse w-36 h-3 bg-light_grey"></div>
            }
          >
          ...
          <div className="grid 2xl:grid-cols-5 gap-x-8 gap-y-8 my-10 xl:grid-cols-4">
            {filteredProducts.map((product, index) => (
              <ProductCard product={product} type="horizontal" key={index} />
            ))}
          </div>
        </Fragment>
      )}
...
export default function SearchResultsPage() {
  return (
    <Suspense
      fallback={
        <div className="my-40 mx-20 h-screen animate-pulse bg-light_grey"></div>
      }
    >
      <SearchResults />
    </Suspense>
